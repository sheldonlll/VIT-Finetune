...
from constants import all_invalid_path
get_model_from_name = {
...
def fit_one_epoch(net, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            with torch.no_grad():
                # print(f"{targets[0]} - {torch.argmax(F.softmax(outputs, dim=-1), dim=-1)[0]}")
                accuracy = torch.mean(
    ...
    torch.save(model.state_dict(), './model_data/densenet121/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' % (
...
# datasets\diatom species datasets (seven data augmentation)\diatom species datasets (seven data augmentation)\train\Eunotia enischna\Eunotia enischna_4_brighter.jpg
# ----------------------------------------#
#   主函数
# ----------------------------------------#
if __name__ == "__main__":
    ...
    # ------------------------------------------#
    # # 加快模型训练的效率
    model_path = "./model_data/densenet121/Epoch59-Total_Loss0.3557-Val_Loss0.9634.pth"
    ...
    # lines = []
    # import os
    # data_path = "./datasets/diatom species datasets (seven data augmentation)/diatom species datasets (seven data augmentation)"
    # for root, dirs, files in os.walk(data_path):
    #     for file in files:
    #         lines.append(os.path.relpath(os.path.join(root, file), data_path))
    ...
    all_invalid_path = "".join(all_invalid_path)
    with open("invalid_path.txt", "w", encoding="utf-8") as f:
        f.write(all_invalid_path)